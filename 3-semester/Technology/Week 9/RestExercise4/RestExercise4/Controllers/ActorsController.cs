...
namespace RestExercise1.Controllers
{
    ...
    public class ActorsController : ControllerBase
    {
        ...
        public IActionResult Get([FromHeader] int startIndex, [FromHeader] int amount)
        {
            ...
            if (actors.Any())
            {
                if (startIndex > actors.Count)
                {
                    return BadRequest("Invalid header input: startIndex is greater than the size of the collection.");
                }
                else if (startIndex >= 0 && amount > 0)
                {
                    Response.Headers.Append("totalAmount", actors.Count().ToString());
                    return Ok(actors.GetRange(startIndex, amount));
                }
                else
                {
                    return BadRequest("Invalid header input: startIndex must be equal to or greater than 0, and amount must be greater than 0.");
                }
            }
